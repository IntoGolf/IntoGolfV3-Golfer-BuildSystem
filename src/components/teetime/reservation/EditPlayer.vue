<template>
  <q-card>
    ...
    <q-card-section>
      ...
      <qrcode-stream v-if="scan" @decode="onDecode" @init="onInit" />
    </q-card-section>
...
export default {
  ...
  data: function () {
    return {
      ...
      messageCancel:
        "U annuleert de deelname van deze speler en kunt hierna een andere speler aan de flight toevoegen, wilt u doorgaan?",
      textIntroMessage:
        "Wenst u deze speler te introduceren voer dan het GSN nummer van de speler in.",
      textExceedIntroodMessage:
        "Deze speler is over de introductie limiet van " +
        this.IntroMax +
        " intoducties en kan nier meer worden geintroduceerd.",
      textExceedIntrosMessage:
        "U kunt niet meer introduceren u heeft het maximum aantal van " +
        this.flight.IntroMax +
        " introducties bereikt.",
      textIntroodMessage:
        'U introduceert deze speler, voordat u start moet het GSN nummer gevalideerd worden. U kunt dit doen door op onderstaande knop "SCAN NGF" te klikken of bij de caddiemaster."',
    };
  },
  computed: {
    canCancel: function () {
    ...
    showIntroMessage: function () {
      return (
        this.flpIntro === null &&
        this.flight.IntroMax > this.flight.IntroCount &&
        this.IntroMax > this.Intros
      );
    },
    showExceedIntroodMessage: function () {
      return (
        this.flpGsn !== null &&
        this.flpGsn.length === 10 &&
        this.IntroMax > 0 &&
        this.IntroMax < this.Intros
      );
    },
    showExceedIntrosMessage: function () {
      return (
        this.flpIntro === null && this.flight.IntroMax <= this.flight.IntroCount
      );
    },
